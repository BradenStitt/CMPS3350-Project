...
void render()
{
	...
	glTranslatef(pf.pos[0], pf.pos[1], 0.0f);
	glBegin(GL_QUADS);
		glVertex2f(-pf.width, -pf.height);
		glVertex2f(-pf.width,  pf.height);
		glVertex2f( pf.width,  pf.height);
		glVertex2f( pf.width, -pf.height);
	glEnd();
	...
	// Draw another platform on top of the existing one
	glPushMatrix(); 
	glColor3ub(250, 20, 20);
	glTranslatef(pf.pos[0] + 100, pf.pos[1] + 50, 0.0f); // Move the platform up by 50 units and to the right by 100 units
	glBegin(GL_QUADS);
		glVertex2f(-pf.width, -pf.height);
		glVertex2f(-pf.width,  pf.height);
		glVertex2f( pf.width,  pf.height);
